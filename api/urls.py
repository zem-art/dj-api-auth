...
from .views import UserViewSet, GroupViewSet, UserRegisterView, UserViewSets, UserViewSetJWT, TodoViewSets, ImageUploadTodo
...
router.register(r'todos/image', ImageUploadTodo, basename='todo_imager')
...
urlpatterns = [
    ...
    ## UPLOAD IMAGE TODO
    path('todo/upload/', ImageUploadTodo.as_view({'post' : 'create'}), name='todo_upload'),

    ## TODO
    path('todo/all/list/', TodoViewSets.as_view({'get' : 'list'}), name='todos_list'),
    path('todo/all/list/delete/', TodoViewSets.as_view({'get' : 'list_delete_todos'}), name='todos_list_delete'),
    ...
    re_path(r'^todo/(?P<uid>[a-zA-Z0-9]+)/delete/$', TodoViewSets.as_view({'delete' : 'delete'}), name='todo_delete'),
]